...
int _is_exist_handler(int **object_list, int size, int current_handler)
{
	...
	for (i = 0; i < size; i++) {
		if ((*object_list)[i] == current_handler)
			return 1;
	}
	...
	uint32_t format, uint32_t parent, uint32_t **object_list, uint32_t *object_num)
{
	...
	if (!ptp_operation_issupported(params, PTP_OC_GetObjectHandles)) {
		add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
			"LIBMTP_Get_Object_List: LIBMTP_Get_Object_List not supported");
		return -1;
	}
	...
	if (currentHandles.n == 0) {
		add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
			"LIBMTP_Get_Object_List: object handle is not exist");
		return -3;
	}
	...
	*object_list = (uint32_t *)malloc(currentHandles.n * sizeof(int));
	...
	for (i = 0; i < currentHandles.n; i++) {
		...
		(*object_list)[temp] = currentHandles.Handler[i];
		temp++;
	}
...
MTPObjectInfo *LIBMTP_Get_Object_Info(LIBMTP_mtpdevice_t *device, uint32_t object_id)
{
	...
	if (!ptp_operation_issupported(params, PTP_OC_GetObjectInfo)) {
		add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
			"LIBMTP_Get_Object_info: LIBMTP_Get_Object_info not supported");
		return NULL;
	}
...
int LIBMTP_Get_Num_Objects(LIBMTP_mtpdevice_t *device, uint32_t format, uint32_t *object_num)
{
	PTPParams *params = (PTPParams *)device->params;
	uint16_t ret;

	if (!ptp_operation_issupported(params, PTP_OC_GetNumObjects)) {
		add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
			"LIBMTP_Get_Num_Objects: LIBMTP_Get_Num_Objects not supported");
		return -1;
	}

	ret = ptp_getnumobjects(params, PTP_GOH_ALL_STORAGE, map_libmtp_type_to_ptp_type(format),
							PTP_GOH_ROOT_PARENT, object_num);
